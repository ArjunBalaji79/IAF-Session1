import json
qa = json.load(open("s4_qa_bundles.json"))
JS = r'''export const meta = {
  name: 'qa-iaf-s4',
  description: 'Adversarially QA each IAF Session-4 slide vs its source: fidelity, overflow, legibility, image quality, constraints',
...
const RULES = `
You are an adversarial QA reviewer for the ImpactAI Foundry (IAF) "Session 4: Bringing Your AI Into the World" deck — an IAF-restyled version of a source Decoded Futures deck. Each slide: n, source_text (original ground-truth lines), records (what was built), new_png (path — READ it).
...
const results = await parallel(chunks.map((chunk)=>()=>
  agent(prompt(chunk), { label:`s4qa:${chunk[0].n}-${chunk[chunk.length-1].n}`, phase:'QA', schema: SCHEMA })));
...
open("workflows_s4_qa.js","w").write(JS)
print("wrote workflows_s4_qa.js", round(len(JS)/1024,1), "KB")
