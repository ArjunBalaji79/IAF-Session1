...
import json
bundles = json.load(open("s4_slide_bundles.json"))
...
const RULES = `
You are mapping slides from a Decoded Futures session deck (cover reads "Session 3 — Bringing Your AI Into the World"; it reviews Session 3, so for IAF this is SESSION 4) into the ImpactAI Foundry (IAF) design system. Each bundle: n, text (shapes→lines with lvl indent), images (file+geometry), src_png (READ it to see layout), char_count.
...
2. RE-BRAND DECODED FUTURES → IMPACTAI FOUNDRY. This deck is rebuilt FOR IAF. NEVER output "Decoded Futures" / "decodedfutures" / "Tech:NYC" / "@technyc" anywhere visible — replace any such VISIBLE mention with "ImpactAI Foundry" (e.g. "read the latest Decoded Futures updates" → "read the latest ImpactAI Foundry updates"). Drop Decoded Futures logos. The cover = arch:cover, eyebrow "IMPACTAI FOUNDRY · SESSION 4", title "Bringing Your AI Into the World", subtitle from the slide. The "REVIEWING SESSION 3" recap slide is correct as-is (it reviews the PRIOR session). Replace any DF contact email / survey URL with IAF-generic phrasing and add a "TODO: set real …" note.
...
4. DEDUPE ANIMATION BUILDS — IMPORTANT, this deck has MANY. Long runs of consecutive slides are progressive-reveal builds of ONE slide. The biggest: the "TRAPPED IN THE CHAT" series (slides ~12–18, 27, 28, 30, 31, 39, 40) and the "Getting Data Into Claude" series (20–23) are each ONE concept revealed step by step. Collapse each such run into ONE record using the FULLEST (usually last) slide, with src = the LAST slide number of that run; SKIP the earlier partial duplicates (emit no record). Note the merge in "note". Be aggressive — the final deck should have far fewer than 57 slides.
5. WORKFLOWS / NODES / DATA-FLOW → use arch:flow. Node-anatomy (Trigger/Actor/Inputs/Action/Success), getting-data-in / getting-data-out chains, and any left→right stage diagram are flow diagrams: put each stage in steps:[{label, sub}]. For a detailed single-node breakdown, two_col or numbered is fine if clearer — pick what reads best from src_png.
...
const results = await parallel(chunks.map((chunk) => () =>
  agent(buildPrompt(chunk), { label: `s4map:${chunk[0].n}-${chunk[chunk.length-1].n}`, phase: 'Map', schema: SCHEMA })
));
const all = [];
...
open("workflows_s4_map.js", "w").write(JS)
print("wrote workflows_s4_map.js", round(len(JS)/1024,1), "KB")
